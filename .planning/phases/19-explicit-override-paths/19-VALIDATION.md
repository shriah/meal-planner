...
- scoped `require_one` rules can intentionally produce incompatible curry/base pairings
- outside manual/locked and scoped `require_one` overrides, default compatibility remains strict
...
| CURRY-06 | Scoped `require_one` can override compatibility intentionally while default flows remain compatibility-first | `npx vitest run src/services/generator.test.ts` |
