...
- one dedicated backbone regression proves migration-shaped data, default generation, explicit overrides, and locked/manual preservation follow one contract
...
|---|---|---|
| CURRY-08 | Migration, library, generator, picker, and explicit override seams all prove the same curry compatibility contract through one backbone regression and supporting focused tests | `npx vitest run src/services/curry-compatibility-regression.test.ts src/services/food-db.test.ts src/services/generator.test.ts src/components/plan/MealPickerSheet.test.tsx src/stores/plan-store.test.ts src/components/library/ComponentForm.test.tsx src/components/library/ComponentRow.test.tsx` |
...
```bash
npx vitest run src/services/curry-compatibility-regression.test.ts
...
rg -n "status: pending|CURRY-08|curry-compatibility-regression.test.ts|MealPickerSheet.test.tsx|plan-store.test.ts|ComponentForm.test.tsx|ComponentRow.test.tsx|npm test" .planning/phases/20-compatibility-regression-coverage/20-VALIDATION.md
```

### Phase gate

```bash
npx vitest run src/services/curry-compatibility-regression.test.ts src/services/food-db.test.ts src/services/generator.test.ts src/components/plan/MealPickerSheet.test.tsx src/stores/plan-store.test.ts src/components/library/ComponentForm.test.tsx src/components/library/ComponentRow.test.tsx
npm test
```

## Wave 0 Coverage

- [ ] `src/services/curry-compatibility-regression.test.ts` proves the milestone backbone contract across migration-shaped data, default generation, explicit overrides, and locked/manual preservation
...
- Pending execution for `src/services/curry-compatibility-regression.test.ts`
